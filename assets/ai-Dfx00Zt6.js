...
이 시간 정보를 바탕으로 아침인사, 점심, 저녁, 심야 등에 맞게 자연스럽게 대화하세요.`,d=(o===`kangel`||o===`streaming`?r:n)+u,f=a.slice(-50);if(l===`openai`){let e=f.map(e=>({role:e.sender===`user`?`user`:`assistant`,content:e.text}));e.unshift({role:`system`,content:d}),e.push({role:`user`,content:i});let n={model:`gpt-4o-mini`,messages:e,temperature:.9,max_tokens:2e3},r=new AbortController,a=setTimeout(()=>r.abort(),15e3),o=await fetch(`https://api.openai.com/v1/chat/completions`,{method:`POST`,headers:{"Content-Type":`application/json`,Authorization:`Bearer ${c}`},body:JSON.stringify(n),signal:r.signal});if(clearTimeout(a),!o.ok){let e=await o.json().catch(()=>({}));throw Error(t(o.status,e))}return(await o.json()).choices[0].message.content}else{let e=f.map(e=>({role:e.sender===`user`?`user`:`model`,parts:[{text:e.text}]}));e.push({role:`user`,parts:[{text:i}]});let n=`https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash:generateContent?key=${c}`,r={system_instruction:{parts:[{text:d}]},contents:e,generationConfig:{temperature:.9,maxOutputTokens:2e3},safetySettings:[{category:`HARM_CATEGORY_HARASSMENT`,threshold:`BLOCK_NONE`},{category:`HARM_CATEGORY_HATE_SPEECH`,threshold:`BLOCK_NONE`},{category:`HARM_CATEGORY_SEXUALLY_EXPLICIT`,threshold:`BLOCK_NONE`},{category:`HARM_CATEGORY_DANGEROUS_CONTENT`,threshold:`BLOCK_NONE`}]},a=new AbortController,o=setTimeout(()=>a.abort(),15e3),s=await fetch(n,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(r),signal:a.signal});if(clearTimeout(o),!s.ok){let e=await s.json().catch(()=>({}));throw Error(t(s.status,e))}return(await s.json()).candidates[0].content.parts[0].text}}export{i as fetchAIChat};